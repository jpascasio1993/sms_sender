...
void main() {
  ...
  setUp(() {
    ...
    outboxBloc = OutboxBloc(
        ...
        getOutboxFromRemote: mockGetOutboxFromRemote,
        updateOutbox: mockUpdateOutbox);
  ...
  group('[OUTBOX] presentation/bloc Outbox BLOC GetOutboxEvent ::', () {
    test('initial state should be empty list', () async {
      expect(
          outboxBloc.initialState, InitialOutboxState(outboxList: emptyList));
    });
    ...
    test('should fail getting outbox', () async {
      //arrange
      when(mockGetOutbox(any)).thenAnswer(
          (_) async => Left(LocalFailure(message: localErrorMessage)));

      //assert
      final expectedCalls = [
        InitialOutboxState(outboxList: emptyList),
        OutboxErrorState.copyWith(
            state: outboxBloc.state, message: localErrorMessage)
      ];
    ...
    test('should get outbox list', () async {
      ...
      final expectedCalls = [
        InitialOutboxState(outboxList: emptyList),
        OutboxLoadingState.copyWith(state: outboxBloc.state),
        RetrievedOutboxState.copyWith(
            state: outboxBloc.state, outboxList: outboxList)
      ];
  ...
  group('[OUTBOX] presentation/bloc LoadMoreEvent', () {
    test('should load more outbox messages', () async {
      ...
      final expectedCalls = [
        ...
        OutboxLoadingState.copyWith(state: outboxBloc.state),
        RetrievedOutboxState.copyWith(
            state: outboxBloc.state,
            outboxList: outboxBloc.state.outboxList + outboxList)
      ];
  ...
  group('[OUTBOX] presentation/bloc GetOutboxFromRemoteAndSaveToLocalEvent',
    ...
    test('should fetch from outbox from remote and save to local', () async {
      // arrange
      when(mockGetOutboxFromRemote(any))
          .thenAnswer((_) async => Right(outboxList));
      ...
      final expectedCalls = [
        ...
        RetrievedOutboxState.copyWith(
            state: outboxBloc.state,
            outboxList: outboxBloc.state.outboxList + outboxList2)
      ];
    ...
    test('should return ServerFailure when failed fetching outbox from remote',
        () async {
      // arrange
      when(mockGetOutboxFromRemote(any)).thenAnswer(
          (_) async => Left(ServerFailure(message: remoteErrorMessage)));
      when(mockGetOutbox(any)).thenAnswer((_) async => Right(outboxList2));
      // assert
      final expectedCalls = [
        InitialOutboxState(outboxList: emptyList),
        OutboxLoadingState.copyWith(state: outboxBloc.state),
        OutboxErrorState.copyWith(
            state: outboxBloc.state, message: remoteErrorMessage)
      ];
    ...
    test('should return LocalFailure when saving remote data to local',
        () async {
      // arrange
      when(mockGetOutboxFromRemote(any)).thenAnswer(
          (_) async => Left(LocalFailure(message: localErrorMessage)));
