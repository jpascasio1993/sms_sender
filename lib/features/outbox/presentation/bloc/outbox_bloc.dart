...
class OutboxBloc extends Bloc<OutboxEvent, OutboxState> {
  ...
  OutboxBloc({@required this.getOutbox, @required this.getOutboxFromRemote}): assert(getOutbox != null);
  ...
  Stream<OutboxState> mapEventToState(OutboxEvent event) async* {
    if(event is GetOutboxEvent) {
      ...
      yield res.fold(
        (failure) => OutboxErrorState.copyWith(state: state, message: failure.message), 
      (outboxList) => RetrievedOutboxState.copyWith(state: state, outboxList: state.outboxList + outboxList));
    }
    else if(event is GetOutboxFromRemoteAndSaveToLocalEvent) {
      final res = await getOutboxFromRemote(OutboxParams(limit: 0, offset: 0));
      final res2 = await getOutbox(OutboxParams(limit: event.limit ?? 0, offset: state.outboxList.length));
